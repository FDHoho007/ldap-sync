...
#Mailman3Provider(config["mailman3"])
#providers = [GitHubProvider(config["github"]), GitLabProvider(config["gitlab"]), MattermostProvider(config["mattermost"]), RedmineProvider(config["redmine"]), StudIPProvider(config["studip"])]
providers = [Mailman3Provider(config["mailman3"])]
